...
pip install future
apt-get --yes install libqjson-dev libqca2-dev libqca2-plugin-ossl libqscintilla2-dev cmake-curses-gui cmake-qt-gui gdal-bin python-gdal python-qscintilla2 git
...
cmake .. -DCMAKE_BUILD_TYPE=Release -DBUILD_PLUGIN_PCL=ON -DBUILD_PLUGIN_HEXBIN=ON -DBUILD_PLUGIN_P2G=ON -DBUILD_PLUGIN_PGPOINTCLOUD=ON -DBUILD_PLUGIN_PYTHON=ON -DBUILD_PLUGIN_SQLITE=ON -DWITH_APPS=ON -DWITH_GEOTIFF=ON -DWITH_LASZIP=ON -DWITH_COMPLETION=ON -DJSONCPP_INCLUDE_DIR=/usr/include/jsoncpp
make -j2
make install
cd ~
