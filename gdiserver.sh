...
cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local/qgis_master -DCMAKE_INSTALL_RPATH=/usr/local/qgis_master/lib -DENABLE_TESTS=OFF -DWITH_SERVER=OFF -DWITH_CUSTOM_WIDGETS=ON -DWITH_PYSPATIALITE=ON -DWITH_QSPATIALITE=ON -DGEOS_CONFIG:FILEPATH=/usr/bin/geos-config -DGEOS_CONFIG_PREFER_PATH:STRING=/bin -DGEOS_CPP_LIBRARY:STRING=/usr/lib/libgeos.so -DGEOS_CPP_LIB_NAME_WITH_PREFIX:STRING=-lgeos -DGEOS_INCLUDE_DIR:PATH=/usr/include -DGEOS_LIBRARY:STRING=/usr/lib/libgeos_c.so -DGEOS_LIB_NAME_WITH_PREFIX:STRING=-lgeos_c
#cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local/qgis_master -DCMAKE_INSTALL_RPATH=/usr/local/qgis_master/lib -DENABLE_TESTS=OFF -DWITH_SERVER=OFF -DWITH_CUSTOM_WIDGETS=ON -DWITH_PYSPATIALITE=ON -DWITH_QSPATIALITE=ON -DGEOS_CONFIG:FILEPATH=/usr/local/libgeos/bin/geos-config -DGEOS_CONFIG_PREFER_PATH:STRING=/bin -DGEOS_CPP_LIBRARY:STRING=/usr/local/libgeos/lib/libgeos.so -DGEOS_CPP_LIB_NAME_WITH_PREFIX:STRING=-lgeos -DGEOS_INCLUDE_DIR:PATH=/usr/local/libgeos/include -DGEOS_LIBRARY:STRING=/usr/local/libgeos/lib/libgeos_c.so -DGEOS_LIB_NAME_WITH_PREFIX:STRING=-lgeos_c
make -j2
make install
cd ~
